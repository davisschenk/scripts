...
code_re = re.compile(r"https?:\/\/secure\.runescape\.com\/m=forum\/sl=0\/forums\?((\d+,)+\d+).*( - (\d+))?")
...
def get_page(url, page, get_page=False):
    try:
...
def from_code(args):
    ...
    last_page = get_page(url, args.start, get_page=True)
    ...
    for code in fileinput.input(files=args.files or "-"):
        ...
        with open(f"out/{file_name}.txt", "w") as f:
            with redirect_stdout(f):
                last_page = get_page(url, 1, get_page=True)
...
if __name__ == "__main__":
    ...
    from_file_p.add_argument("files", metavar="FILE", nargs="*", help="Files to read, if empty use stdin")
    from_file_p.set_defaults(func=from_file)

    from_code_p = subparsers.add_parser("from_code")
    from_code_p.add_argument("code", type=str, help="Enter quick find code, found on bottom of forum post")
